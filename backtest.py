...
raw_data = {}
# FIXED: Adjusted start date to late 2004 when VNQ launched to prevent silent dropna() truncation
...
# FIXED: Added protect against divide-by-zero crashes in backtest RSI
...
for idx in range(lookback_period, len(df_universe)):
    ...
    # REBALANCING TRIGGER: Triggers weekly
    if last_rebalanced_week is None or current_year_week != last_rebalanced_week:
        ...
        # FIXED: Removed the asset change condition to rebalance weights EVERY week, correcting portfolio drift
        total_cash = 0
        if current_holdings:
